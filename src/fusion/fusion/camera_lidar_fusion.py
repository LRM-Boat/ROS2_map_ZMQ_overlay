...
class LidarColorProjector(Node):
    ...
    def process_and_publish_colored_pointcloud(self):
        ...
        if self.current_pointcloud is None:
            self.get_logger().warn("Облако точек ещё не получено.")
            return

        if not isinstance(self.current_pointcloud, PointCloud2):
            self.get_logger().error("Неверный тип self.current_pointcloud.")
            return
        pts_xyz = np.array(
            [[p[0], p[1], p[2]]
        ...
        for i, (u, v) in enumerate(img_pts):
            ...
            if not (0 <= u_i < w and 0 <= v_i < h):
                continue  # Пропускаем точки вне изображения
